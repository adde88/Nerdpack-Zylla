...
local Survival = {
	...
	{'#152615', 'item(152615).usable&item(152615).count>0&health<=UI(AHP_spin)&UI(AHP_check)'}, 													-- Astral Healing Potion
	{'#127834', 'item(152615).count==0&item(127834).usable&item(127834).count>0&health<=UI(AHP_spin)&UI(AHP_check)'}, 		-- Ancient Healing Potion
	{'#5512', 'item(5512).usable&item(5512).count>0&health<=UI(HS_spin)&UI(HS_check)'}, 																	-- Health Stone
}

local Interrupts = {
	{'!Consume Magic', 'range<=20'},
	{'!Chaos Nova', 'UI(chaos_check)&inMelee&spell(Consume Magic).cooldown>gcd&!player.lastcast(Consume Magic)'}
}
...
local Cooldowns = {
	...
	{'#144259', 'UI(kj)&range<=40&area(10).enemies>=UI(kiljaeden)&equipped(144259)'}, 		--XXX: Kil'jaeden's Burning Wish
...
local inCombat = {
	...
	{Cooldowns, 'toggle(cooldowns)', 'target'},
	{Mythic_Plus, 'inMelee'},
	{xSTCombat, 'inMelee&player.area(15).enemies<3', 'target'},
	{xAoECombat, 'inMelee&player.area(15).enemies>2&toggle(aoe)', 'target'},
	{Ranged, nil, 'target'},
