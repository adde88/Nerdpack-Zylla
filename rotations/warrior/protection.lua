...
local Etc = {
	{'Impending Victory', '{!player.buff(Victorious)&player.rage>10&player.health<95}||{player.buff(Victorious)&player.health<80}'},
	{'Heroic Throw', '!target.inMelee&target.range<40&target.inFront'},
...
local Interrupts = {
	...
	{'!Spell Reflection', '{target.inMelee&spell(Pummel).cooldown>gcd&!prev_gcd(Pummel)}||{target.range>00&!spell(Pummel).cooldown}'}
}
...
local PreCombat = {
	{'Intercept', '!target.inMelee&target.range<25&target.enemy&!prev_gcd(Heroic Leap)&UI(kIntercept)&target.alive'},
	{'Heroic Throw', '!target.inMelee&target.range<40&target.inFront&target.alive&target.enemy&!UI(kIntercept'}
}

local Something = {
	{'Focused Rage', 'talent(3,2)&player.buff(Ultimatum)&!talent(6,1)'},
	{'Battle Cry', '{talent(6,1)&talent(3,2)&spell(Shield Slam).cooldown<5.5-gcd}||!talent(6,1)'},
...
local ST = {
	...
	{'Focused Rage', '{player.buff(Vengeance: Focused Rage)&!player.buff(Vengeance: Ignore Pain)}||{player.buff(Ultimatum)&player.buff(Vengeance: Focused Rage)&!player.buff(Vengeance: Ignore Pain)}||{talent(6,1)&player.buff(Ultimatum)&!player.buff(Vengeance: Ignore Pain)&!player.buff(Vengeance: Focused Rage)}||{talent(6,1)&!player.buff(Vengeance: Ignore Pain)&!player.buff(Vengeance: Focused Rage)&player.rage>20}||{player.buff(Ultimatum)&player.buff(Vengeance: Ignore Pain)&spell(Shield Slam)&player.rage<10}||{player.rage>000}'},
	...
	{AoE, 'toggle(aoe)&player.area(8).enemies>1'},
	{Something},
