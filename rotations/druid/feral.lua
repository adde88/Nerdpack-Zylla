...
local SBT_Opener = {
	...
	{'Tiger\'s Fury', '!target.dot(Rip).ticking&player.combopoints==5'},
}
...
local Finisher = {
	...
	{'Ferocious Bite', 'energy.deficit==0&player.combopoints==5&{energy.time_to_max<1||player.buff(Berserk)||player.buff(Incarnation: King of the Jungle)||player.spell(Tiger\'s Fury).cooldown<3||{talent(6,2)&player.buff(Clearcasting)}}'},
}

local Generator = {
	{'Brutal Slash', 'talent(6,2)player.combopoints<5'},
	{'!Ashamane\'s Frenzy', 'player.combopoints<3&toggle(Cooldowns)&{player.buff(Bloodtalons)||!talent(7,2)}&{player.buff(Savage Roar)||!talent(6,3)}'},
	{'Elune\'s Guidance', 'talent(6,3)&{player.combopoints==0&player.energy<action(Ferocious Bite).cost+25-energy.regen*player.spell(Elune\'s Guidance).cooldown}'},
	{'Elune\'s Guidance', 'talent(6,3)&{player.combopoints==0&player.energy>=action(Ferocious Bite).cost+25}'},
	{Thrash_Pool, 'talent(7,1)&player.area(8).enemies.infront>8'},
	{Swipe_Pool, 'player.area(8).enemies.infront>5'},
	{Rake_Pool, 'player.combopoints<5&{!target.dot(Rake).ticking||{!talent(7,2)&target.dot(Rake).remains<target.dot(Rake).duration*0.3}||{talent(7,2)&player.buff(Bloodtalons)&{!talent(5,1)&target.dot(Rake).remains<8||target.dot(Rake).remains<6}&persistent_multiplier(Rake)>dot(Rake).pmultiplier*0.80}}&target.dot(Rake).remains>dot(Rake).tick_time'},
	...
	{'Swipe', 'player.combopoints<5&player.area(8).enemies.infront>2'},
	{'Shred', 'player.combopoints<5&{player.area(8).enemies.infront<3||talent(7,1)}'},
}
